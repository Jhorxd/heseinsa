...
class Producto_model extends model{
    ...
    // obtener producto con unidad de medida
    public function getProductoConUM($producto){
        $sql = "SELECT p.*, um.UNDMED_Codigo, um.UNDMED_Descripcion, um.UNDMED_Simbolo
                    FROM cji_producto p
                    INNER JOIN cji_productounidad pu ON pu.PROD_Codigo = p.PROD_Codigo                
                    INNER JOIN cji_unidadmedida um ON um.UNDMED_Codigo = pu.UNDMED_Codigo
                    WHERE p.PROD_Codigo = $producto
                ";
        $query = $this->db->query($sql);
    ...
    public function getProductoCodigo($codigo){
        $sql = "SELECT p.*
                    FROM cji_producto p
                    WHERE p.PROD_CodigoUsuario = '$codigo' LIMIT 1;
                ";
        $query = $this->db->query($sql);
    ...
    */
    ...
    public function listar_producto_stockmin( $filter = NULL ){
        ...
        $sql = "SELECT p.PROD_Nombre, p.PROD_FlagBienServicio, p.PROD_StockMinimo, p.PROD_StockMaximo, p.PROD_CodigoInterno, p.PROD_CodigoUsuario, p.PROD_CodigoOriginal, p.PROD_Modelo, p.PROD_GenericoIndividual, p.PROD_UltimoCosto, p.PROD_FlagEstado, p.AFECT_Codigo, p.FAMI_Codigo, ap.ALMPROD_Stock
                    FROM cji_producto p
                    INNER JOIN cji_productocompania pc ON pc.PROD_Codigo = p.PROD_Codigo
