...
public function countFilas($tipo_oper, $filter = NULL, $number_items = '', $offset = ''){
    ...
    public function reportGuia($date){
        $this->db->select("g.GUIAREMP_Codigo, g.GUIAREMC_Serie, g.GUIAREMC_Numero, g.GUIAREMC_Fecha, g.GUIAREMC_FlagEstado, g.GUIAREMC_PesoTotal, g.GUIAREMC_TipoOperacion");
        $this->db->select("g.GUIAREMC_FechaTraslado, g.GUIAREMC_RucEmpresaTransp, g.GUIAREMC_EmpresaTransp, g.GUIAREMC_Placa, g.GUIAREMC_NombreConductor");
        $this->db->select("g.GUIAREMC_ConductorApellido, g.GUIAREMC_PersReceDNI, g.GUIAREMC_Licencia, g.GUIAREMC_UbigeoPartida, g.GUIAREMC_PuntoPartida");
        $this->db->select("g.GUIAREMC_UbigeoLlegada, g.GUIAREMC_PuntoLlegada, g.GUIAREMC_Observacion");
        $this->db->select("gd.GUIAREMDETC_Descripcion, gd.PRODCTOP_Codigo, gd.GUIAREMDETC_Cantidad");

        $this->db->select("a.ALMAC_Descripcion");

        $this->db->select("um.UNDMED_Simbolo");
        
        $this->db->select("CASE c.CLIC_TipoPersona WHEN 0 THEN p.PERSC_NumeroDocIdentidad WHEN 1 THEN e.EMPRC_Ruc ELSE '' END as numero", FALSE);
		$this->db->select("CASE c.CLIC_TipoPersona WHEN 0 THEN CONCAT_WS(' ', p.PERSC_Nombre, p.PERSC_ApellidoPaterno, p.PERSC_ApellidoMaterno) WHEN 1 THEN e.EMPRC_RazonSocial ELSE '' END as razon_social", FALSE);
		$this->db->select("CASE pro.PROVC_TipoPersona WHEN 0 THEN prop.PERSC_NumeroDocIdentidad WHEN 1 THEN proe.EMPRC_Ruc ELSE '' END as numero_proveedor", FALSE);
		$this->db->select("CASE pro.PROVC_TipoPersona WHEN 0 THEN CONCAT_WS(' ', prop.PERSC_Nombre, prop.PERSC_ApellidoPaterno, prop.PERSC_ApellidoMaterno) WHEN 1 THEN proe.EMPRC_RazonSocial ELSE '' END as razon_social_proveedor", FALSE);

        $this->db->from("cji_guiarem AS g");
        $this->db->join("cji_almacen AS a", "g.ALMAP_Codigo = a.ALMAP_Codigo");
        $this->db->join("cji_guiaremdetalle AS gd", "g.GUIAREMP_Codigo = gd.GUIAREMP_Codigo");
        $this->db->join("cji_productounidad AS pu", "gd.PRODCTOP_Codigo = pu.PROD_Codigo");
        $this->db->join("cji_unidadmedida AS um", "pu.UNDMED_Codigo = um.UNDMED_Codigo");
		$this->db->join('cji_cliente as c', 'c.CLIP_Codigo = g.CLIP_Codigo', 'left');
		$this->db->join('cji_persona as p', 'p.PERSP_Codigo = c.PERSP_Codigo', 'left');
		$this->db->join('cji_empresa as e', 'e.EMPRP_Codigo = c.EMPRP_Codigo', 'left');
		$this->db->join('cji_proveedor as pro', 'pro.PROVP_Codigo = g.PROVP_Codigo', 'left');
		$this->db->join('cji_persona as prop', 'prop.PERSP_Codigo = pro.PERSP_Codigo', 'left');
        $this->db->join('cji_empresa as proe', 'proe.EMPRP_Codigo = pro.EMPRP_Codigo', 'left');

        $this->db->where("gd.GUIAREMDETC_FechaRegistro >=", $date->startDate);
        $this->db->where("gd.GUIAREMDETC_FechaRegistro <=", $date->endDate);
        $this->db->where("g.COMPP_Codigo", $this->somevar['compania']);

        $this->db->order_by('gd.GUIAREMDETC_FechaRegistro', 'desc');
