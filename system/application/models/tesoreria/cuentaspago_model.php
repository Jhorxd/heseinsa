...
class Cuentaspago_Model extends Model {
    ...
    public function obtainInvoiceRelatedToGuide($documentId){
        $this->db->select('g.GUIAREMC_Serie, g.GUIAREMC_Numero, u.UBIGC_Descripcion, c.CPC_Fecha, c.MONED_Codigo, c.CPC_RetencionPorc, c.CPP_Compracliente, f.FORPAC_Descripcion, cc.CUOT_Fecha');
        $this->db->from('cji_cuentas cu');
        $this->db->join('cji_comprobante c', 'c.CPP_Codigo = cu.CUE_CodDocumento', 'left');
        $this->db->join('cji_comprobante_guiarem cg', 'cu.CUE_CodDocumento = cg.CPP_Codigo', 'left');
        $this->db->join('cji_guiarem g', 'cg.GUIAREMP_Codigo = g.GUIAREMP_Codigo', 'left');
        $this->db->join('cji_ubigeo u', 'g.GUIAREMC_UbigeoLlegada = u.UBIGP_Codigo', 'left');
        $this->db->join('cji_formapago f', 'c.FORPAP_Codigo = f.FORPAP_Codigo', 'left');
        $this->db->join('comprobantes_cuotas cc', 'c.CPP_Codigo = cc.CPP_Codigo', 'left');
        $this->db->where('cu.CUE_CodDocumento', $documentId);
        $this->db->where('cu.CUE_FlagEstado', 1);
