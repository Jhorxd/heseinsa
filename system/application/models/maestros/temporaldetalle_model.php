...
class Temporaldetalle_Model extends Model {
    ...
    public function insertar_productodetalle($filter){
        $this->db->insert('temporal_detalle',$filter);
        $id = $this->db->insert_id();
        return $id;
    }

    public function obtener_producto_temporal($tempdeId, $producto, $session, $lote){
        ...
        $sql = "SELECT td.*,
                ...
                WHERE td.PROD_Codigo = $producto AND td.TEMPDE_SESSION = '$session' AND td.TEMPDE_Codigo = '$tempdeId' $where
                ";
