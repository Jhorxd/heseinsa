...
class Ocompra_model extends Model{
    ...
    public function numero($numero, $serie) {
        ...
        $this->db->where('CPC_FlagEstado', 1);
    ...
    public function obtener_detalle_ocompra($ocompra){
        ...
        $sql = "SELECT oc.*, pr.PROD_CodigoInterno, pr.PROD_CodigoUsuario, pr.PROD_CodigoOriginal, pr.PROD_FlagBienServicio, pr.PROD_Nombre, pr.FABRIP_Codigo, um.UNDMED_Simbolo, m.MARCC_CodigoUsuario, m.MARCC_Descripcion, pr.PROD_FichaTecnica,
    ...
    public function getOcompra($filter = NULL){
    	...
    	$sql = "SELECT DATE_FORMAT(o.OCOMC_FechaRegistro, '%d/%m/%Y') fecha, o.*,
                (SELECT GUIAREMP_Codigo FROM cji_guiarem gr WHERE gr.OCOMP_Codigo = o.OCOMP_Codigo LIMIT 1) as GUIAREMP_Codigo,
                (SELECT CONCAT_WS('-', gr.GUIAREMC_Serie, gr.GUIAREMC_Numero) FROM cji_guiarem gr WHERE gr.OCOMP_Codigo = o.OCOMP_Codigo AND gr.GUIAREMC_FlagEstado != 0 LIMIT 1) as GUIAREMC_SerieNumero,
