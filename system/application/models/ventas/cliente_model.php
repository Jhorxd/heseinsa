...
class Cliente_model extends Model{
  ...
  ## Dev: RC -> Begin
    ...
    public function buscarClienteRuc($keyword, $compania){
        ...
        $sql = "SELECT c.CLIP_Codigo, c.CLIC_TipoPersona, c.EMPRP_Codigo, c.PERSP_Codigo, c.CLIC_Vendedor, c.TIPCLIP_Codigo, c.CLIC_AgenteRetencion,
                        CONCAT_WS(' ', p.PERSC_Nombre, p.PERSC_ApellidoPaterno, p.PERSC_ApellidoMaterno) as PERSC_Nombre, p.PERSC_Ruc, p.PERSC_NumeroDocIdentidad,
    ...
    public function transportistaRuc($keyword,$accion=1){
        if ($accion==1) {
            try {
                $sql = "SELECT t.* FROM transportistas t WHERE t.TRANS_Doc LIKE '%$keyword%'";  
                $query = $this->db->query($sql);
                if ($query->num_rows() > 0) {
                    return $query->result();
                }else{
            ...
            try {
                $sql = "SELECT t.* FROM transportistas t WHERE t.TRANS_Doc = '$keyword' LIMIT 1";  
                $query = $this->db->query($sql);
                if ($query->num_rows() > 0) {
                    return $query->result();
                }else{
    ...
    public function buscarConductorDoc($keyword,$accion=1){
        if ($accion==1) {
            try {
                $sql = "SELECT t.* FROM transportistas t WHERE t.TRANS_DocConductor LIKE '%$keyword%'";  
                $query = $this->db->query($sql);
                if ($query->num_rows() > 0) {
                    return $query->result();
                }else{
            ...
            try {
                $sql = "SELECT t.* FROM transportistas t WHERE t.TRANS_DocConductor = '$keyword' LIMIT 1";   
                $query = $this->db->query($sql);
                if ($query->num_rows() > 0) {
                    return $query->result();
                }else{
    ...
    public function insertar_transportista($filter){
        $this->db->insert("transportistas", (array) $filter);
        return $this->db->insert_id();
    }
    
    
    
    public function actualizar_transportista($id, $filter){
        $this->db->where('ID_Trans',$id);
        return $this->db->update('transportistas', $filter);
    }
