...
class Lib_props
{
  ...
  private   $compania;
  private   $usuario;
  private   $color = "#EEEEEE"; //color de las cabeceras de tablas de la guia de remisión
    ...
    if ($adjunto == true) {
      ...
      if ($tipo == 8 || $tipo == 9 || $tipo == 14) {
        ...
        if ($docs->a4 == 1) {
          ...
          $comprobanteData = $this->ci->comprobante_model->obtener_comprobante($codigo);
          $document = $this->ci->documento_model->obtenerAbreviatura($comprobanteData[0]->CPC_TipoDocumento);

          $mail->AddStringAttachment($doc, $document[0]->DOCUC_Descripcion." HESEINSA ".$comprobanteData[0]->CPC_Serie."-".$comprobanteData[0]->CPC_Numero.".pdf");
        ...
        if ($docs->xml == 1) {
          ...
          $cod_empresa = $this->empresa = $this->ci->session->userdata("empresa");
          $datos = $this->ci->empresa_model->obtener_datosEmpresa($cod_empresa);
          
          if ($xml->respuestas_enlacexml != NULL && $xml->respuestas_enlacexml != "") {
            $ruc= $datos[0]->EMPRC_Ruc;
            $serienumero = $ruc."-"."0".$xml->respuestas_tipoDocumento."-".$xml->respuestas_serie."-".$xml->respuestas_numero;
            $doc = $this->file_get_contents_curl($xml->respuestas_enlacexml);
            $mail->AddStringAttachment($doc, $serienumero. ".xml");
          }
        }
        
        if ($docs->cdr == 1) {
          $cdr = $this->ci->comprobante_model->consultar_respuestaSunat($codigo,"C");
          
          $cod_empresa = $this->empresa = $this->ci->session->userdata("empresa");
          $datos = $this->ci->empresa_model->obtener_datosEmpresa($cod_empresa);
          
          if ($cdr->respuestas_enlacecdr != NULL && $cdr->respuestas_enlacecdr != "") {
            $ruc= $datos[0]->EMPRC_Ruc;
            $serienumero = "R-".$ruc."-". "0".$cdr->respuestas_tipoDocumento."-".$cdr->respuestas_serie."-".$cdr->respuestas_numero;
            $doc = $this->file_get_contents_curl($cdr->respuestas_enlacecdr);
            $mail->AddStringAttachment($doc,$serienumero. ".xml");
  ...
  public function ocompra_pdf($codigo, $flagPdf = 1, $enviarcorreo = false)
  {
    ...
    $nombreComprador = $datos_ocompra[0]->OCOMC_NombreComprador;
    ...
    foreach ($datos_detalle_ocompra as $indice => $valor) {
      ...
      $bgcolor = ($indice % 2 == 0) ? "#FFFFFF" : "#F1F1F1";
      if($valor->PROD_FichaTecnica == ""){
        $ficha = ' - ';

      }else{
        $ficha = '<a href="'. base_url() . $valor->PROD_FichaTecnica .'" style="color:#73b3e5">PDF</a>';
      ...
      $detaProductos = $detaProductos . '
            ...
            <td style="' . $styleBorder . ' text-align:left;">' . $ficha . '</td>
            ...
            <tr>
              <td style="width:7cm;">
                <table cellpadding="0.1cm" border="0" style="text-indent: -0.25cm;">
                    <tr> 
                    <td style="width:1.5cm; font-style:italic; font-weight: bold">Contacto:</td>
                    <td style="text-align: center">' . $contacto[0]->ECONC_Descripcion . '</td>
                  </tr>
                  <tr> 
                  ...
                  <tr> 
                    <td style="width:2cm; font-style:italic; font-weight: bold">Comprador:</td>
                    <td style="text-align: center">' . $nombreComprador . '</td>
                  </tr>
                </table>
              </td>
              <td style="width:12cm;">
    ...
    $productoHTML = '
          ...
          <th style="' . $styleBorder . ' font-style:italic; font-weight:bold; text-align:center; width:8.0cm;">Descripción</th>
          <th style="' . $styleBorder . ' font-style:italic; font-weight:bold; text-align:center; width:1.5cm;">Ficha</th>
  ...
  public function comprobante_pdf_a4($codigo, $flagPdf = 1, $enviarcorreo = false)
  {
    ...
    $storeData = $this->ci->almacen_model->getAlmacen($datos_comprobante[0]->ALMAP_Codigo);
    ...
    $this->pdf->SetMargins(10, 55, 10, 8);
    ...
    foreach ($detalle_comprobante as $indice => $valor) {
      ...
      $detaProductos = $detaProductos . '
                ...
                <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; text-align:left;">' . $codigo_usuario . '</td>
                <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; text-align:left;">' . $nombre_producto . '</td>
                <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; text-align:center;">' . $medidaDetalle . '</td>
    ...
    $clienteHTML = '<table style="text-indent:0cm;" cellpadding="0.02cm" border="0">
      ...
      if($tipo_oper == 'V'){
        $clienteHTML .= '<td style="width:14%; font-weight:bold;">VENDEDOR:</td>
                          <td style="width:15%;">' . $vendedor . '</td>
                          
                        </tr>
                        </table>';
      }else{
        $clienteHTML .= '<td style="width:14%; font-weight:bold;">ALMACEN:</td>
                          <td style="width:15%;">' . $storeData[0]->ALMAC_Descripcion . '</td>
    ...
    $guiaRem = ($guiaRemision != '') ? '<tr>
        <td style="text-align:right; width:4.25cm;"><b>GUIA DE REMISIÓN:</b></td>
        <td style="width:15.5cm;">' . $guiaRemision . '</td>
        </tr>' : '';

    $serieOCHTML = ($serieOC != "") ? '<tr>
        <td style="text-align:right; width:3.35cm;"><b>COTIZACIÓN:</b></td>
        <td style="width:15.5cm;">' . $serieOC . '</td>
        </tr>' : '';

    $ordenCompraClienteHTML = ($ordenCompraCliente != "") ? '<tr>
        <td style="text-align:right; width:4.4cm;"><b>ORDEN DE COMPRA:</b></td>
        <td style="width:15.5cm;">' . $ordenCompraCliente . '</td>
        </tr>' : '';
    $observacionHTML = ($observacion != "") ? '<tr>
        <td style="text-align:right; width:3.65cm;"><b>OBSERVACIÓN:</b></td>
        <td style="width:15.5cm;">' . $observacion . '</td>
        </tr>' : '';
    $tienecuotasHTML = ($tiene_cuotas != "") ? '<tr>
        <td style="text-align:right; width:4.15cm;"><b>CUOTAS DE PAGO:</b></td>
        <td style="width:15.5cm;">' . $tiene_cuotas . '</td>
        </tr>' : '';

    $sujetaretencionDetHTML = ($retencion_codigo != "" && $retencion_codigo != null) ? '<tr>
        <td style="text-align:right; width:1.4cm;"></td>
...
$this->pdf->settingHeaderData($header, $tipoDocumento);
  ...
  public function guiarem_pdf($codigo, $flagPdf = 0, $enviarcorreo = false)
  {
    ...
    $ordenCompra        = $datos_guiarem[0]->DOCUP_Codigo;
    ...
    foreach ($detalle_guiarem as $indice => $valor) {
      ...
      if($valor->show_details == 2){
        ...
        $nombre_producto =  $valor->PROD_Nombre . ". $valor->GUIAREMDETC_Observacion";
        $codigo_usuario = $valor->PROD_CodigoUsuario;
      }
    ...
    $clienteHTML = '
          <table  style="font-size:7pt; border-radius: 1px 1px 1px 1px;" cellpadding="0.05cm" border="0" width="100%">
            <tr bgcolor="'.$this->color.'" style="" >
             <td style="font-weight:bold; text-align: left; color: #000000;">' . $tpCliente . '</td>
            </tr>
            <tr>
          ...
          <tr bgcolor="'.$this->color.'">
          <td style="font-weight:bold; text-align: left; color:#000000;">DATOS DEL TRASLADO</td>
          </tr>
          <tr>
          ...
          <table style="font-size:7pt;" border="0" cellpadding="0.05cm">
          <tr bgcolor="'.$this->color.'">
          <td style="font-weight:bold; text-align: left; color: #000000;" colspan>DATOS DEL TRASLADO</td>
          </tr>
          <tr>
          ...
          <table style="font-size:7pt;" border="0" cellpadding="0.05cm">
          <tr bgcolor="'.$this->color.'">
          <td style="font-weight:bold; text-align: left; color: #000000;">DATOS DEL TRANSPORTE</td>
          </tr>
          <tr>
    ...
    $productoHTML = '<table cellpadding="0.1cm" style="font-size:8pt;">
          ...
          <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; color: #000000; text-align:center; width:10%;"> Nro.</td>
          <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; color: #000000; text-align:center; width:15%;"> CÓDIGO</td>
          <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; color: #000000; text-align:left; width:55%;">DESCRIPCIÓN</td>
          <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; color: #000000; text-align:center; width:10%;"> UND.</td>
          <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; color: #000000; text-align:center; width:10%;"> CANTIDAD</td>
          </tr>
          ' . $detaProductos . '
          </table>';
    $this->pdf->writeHTML($productoHTML, true, false, true, '');
    ...
    $ordenCompraHTML = ($ordenCompra != '') ? '<tr>
              <td style="font-weight:bold; text-align:justification;">Orden de compra: <span style="font-style:italic; font-weight:normal;">' . $ordenCompra . '</span></td><br>
              </tr>' : '';
    ...
    $ordenCompraHTML2 = ($ordenCompra != '') ? '<tr>
              <td style="text-align:center; font-size:11pt; padding:5px;">
                  NO SE ACEPTARAN CAMBIOS NI DEVOLUCIONES, POR PARTE DEL CLIENTE, PASADO LOS 02 DÍAS DE RECEPCIONADA Y ENTREGADA LA MERCADERÍA.
              </td>
          </tr>' : '';
    
    $this->pdf->writeHTML($ordenCompraHTML2, true, false, true, '');
