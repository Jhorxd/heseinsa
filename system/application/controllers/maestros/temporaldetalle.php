...
class Temporaldetalle extends Controller{
                        ...
                        foreach ($datosAlmacenProducto as $key => $valorReal){
                            ...
                            if($almacen != NULL && $almacen != 0 && trim($almacen) != "" ){
                                if($CodigoAlmacenProducto == $almacen){
                                    ...
                                    $ficha_ruta = $valor->PROD_FichaTecnica;
    ...
    /*public function actualizar_ficha(){
            $this->load->library('upload');

            parse_str($this->input->post('info'), $form_data);
            ...
            $upload_path = './uploads/fichas/';
            $file_name = $id . '.pdf';
            $file_path = $upload_path . $file_name;
        
            if (file_exists($file_path)) {
                unlink($file_path); // Eliminar el archivo existente
            }

            $config['upload_path'] = $upload_path;
            $config['allowed_types'] = 'pdf|PDF';
            $config['max_size'] = 2048; // 2MB
            $config['file_name'] = $file_name; // Usar el ID con la extensi籀n .pdf
        
            $this->upload->initialize($config);
        
            if ($this->upload->do_upload('ficha')) {
                $upload_data = $this->upload->data();
                $ficha_path = $upload_data['file_name'];
            } else {
            ...
            $rutadb = 'uploads/fichas/'. $file_name;
     
        # REGISTRO PRODUCTO
            $filter = new stdClass();
            $filter->PROD_FichaTecnica = $rutadb;
    ...
    public function actualizar_ficha() {
        ...
        // Recuperar el dato de la sesi車n
        $codproducto = $this->session->userdata('codproducto');
        $numeroeditar = $this->session->userdata('numeroeditar');
        ...
        switch ($numeroeditar) {
            case 1:
                $id = !empty($form_data['id']) ? $form_data['id'] : $form_data['id_p'];
                break;
            ...
            default:
                echo json_encode(['result' => 'error', 'message' => 'N迆mero de edici車n no v芍lido.']);
                return;
        }
    
        if (!$id) {
            echo json_encode(['result' => 'error', 'message' => 'ID no especificado.']);
            return;
        }
        ...
        if (file_exists($file_path)) {
            if (!unlink($file_path)) {
                echo json_encode(['result' => 'error', 'message' => 'No se pudo eliminar el archivo existente.']);
                return;
            }
        }
        ...
        $config['allowed_types'] = 'pdf|PDF';
        $config['max_size'] = 2048; // 2MB
        $config['file_name'] = $file_name; // Usar el ID con la extensi車n .pdf
        ...
        if ($this->upload->do_upload('ficha')) {
            $upload_data = $this->upload->data();
            $rutadb = 'uploads/fichas/' . $upload_data['file_name'];
        } else {
            $error = $this->upload->display_errors();
            echo json_encode(['result' => 'error', 'message' => $error]);
            return;
        ...
        // Actualizar registro
        if ($id != NULL && $id != "") {
            $filter->PROD_FechaModificacion = null; // Establecer fecha de modificaci車n, si aplica
            $update = $this->producto_model->modificarRegistro($id, $filter);
    
            // Responder seg迆n el resultado de la actualizaci車n
            if ($update) {
                $json = ['result' => 'success'];
            } else {
                $json = ['result' => 'error', 'message' => 'Error al actualizar el registro.'];
            }
        } else {
            $json = ['result' => 'error', 'message' => 'ID no v芍lido.'];
        }
    ...
    public function registrar_prodtemporal(){
        ...
        $showDetails        = ($this->input->post('showDetails') == 'on')? '2' : '1';
        ...
        $filter->show_details   = $showDetails;
    ...
    public function obtener_producto_temporal(){
        $tempdeId      = $this->input->post('tempdeId');
        ...
        $this->session->set_userdata('codproducto', $codproducto);

        $idLote = ( $this->input->post('idLote') == "null" || $this->input->post('idLote') == NULL ) ? 0 : $this->input->post('idLote');

        $result = array();
        $data = $this->temporaldetalle_model->obtener_producto_temporal($tempdeId, $codproducto, $tempSession, $idLote);
...
#####################################################################################
########### SE USA CUANDO SE ASOCIA UN DOCUMENTO. Ejem. Factura de Cotizaci籀n
#####################################################################################
...
########################################################################################################
########### SE USA CUANDO SE EDITA UN DOCUMENTO ASOCIADO. Ejem. Editar Factura relacionada a Cotizaci籀n
########################################################################################################
    ...
    public function introducir_prodtemporal_ocompra($comprobante,$session){
        ...
        if (count($detalle) > 0) {
            foreach ($detalle as $indice => $valor) {
                ...
                $total = $valor->OCOMDEC_Total;
                $pu_conigv = $valor->OCOMDEC_Pu_ConIgv;
                $comprador = $valor ->OCOMC_NombreComprador;
                $descuento = $valor->OCOMDEC_Descuento;
