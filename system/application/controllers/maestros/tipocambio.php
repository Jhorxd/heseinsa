...
class Tipocambio extends Controller{
    ...
    public function ingesar_tipo_cambio(){
        ...
        for ($i = 0; $i < $size; $i++) {
            if ($cambio[$i] != ""){
                $exists = $this->tipocambio_model->tcExists($fecha, $compania, $moneda[$i]); # VERIFICA SI EL TIPO DE CAMBIO YA ESTA REGISTRADO EN ALGUNA COMPAﾃ選A, "SI ESTA EN 1 ESTA EN TODAS".
                ...
                if ($exists == false)
                    $id = $this->tipocambio_model->insertar($filter); # REGISTRA EL TIPO DE CAMBIO EN TODAS LAS COMPAﾃ選AS
                else
                    $id = $this->tipocambio_model->update_tc($filter); # ACTUALIZA EL TIPO DE CAMBIO EN TODAS LAS COMPAﾃ選AS
            }
        }
    ...
    public function ingresar_tipo_cambio_automatico(){
            // Leer y decodificar JSON del body
            $json = file_get_contents('php://input');
            $data = json_decode($json, true);

            // Validar estructura mínima
            if (!isset($data['fecha'], $data['venta'], $data['moneda'])) {
                echo json_encode(["success" => false, "message" => "Datos incompletos."]);
                return;
            }

        //$fecha = date('Y-m-d', strtotime($data['fecha'] . ' +1 day'));
                  $fecha =date('Y-m-d');

            $compania = $this->somevar['compania'];
            $monedaCodigo = strtoupper(trim($data['moneda']));

            // Mapear código moneda a ID
            $monedaMap = [
                'USD' => 2,
                'PEN' => 1,
            ...
            if (!isset($monedaMap[$monedaCodigo])) {
                echo json_encode(["success" => false, "message" => "Moneda no reconocida."]);
                return;
            }

            $monedaDestino = $monedaMap[$monedaCodigo];
            $factorConversion = $data['venta']; // T.C. de venta

            // Validar tipo de cambio
            if (!is_numeric($factorConversion)) {
                echo json_encode(["success" => false, "message" => "Tipo de cambio inválido."]);
                return;
            }

            // Verificar si ya existe
            $exists = $this->tipocambio_model->tcExists($fecha, $compania, $monedaDestino);

            // Construir objeto
            $filter = new stdClass();
            $filter->TIPCAMC_MonedaOrigen = 1; // PEN
            $filter->TIPCAMC_MonedaDestino =  strval($monedaDestino);
            $filter->TIPCAMC_Fecha = $fecha;
            $filter->TIPCAMC_FactorConversion =  strval($factorConversion);
            // var_dump($filter);
            // exit();
            // Insertar o actualizar
            $id = $exists ? $this->tipocambio_model->update_tc($filter) : $this->tipocambio_model->insertar($filter);

            // Respuesta al frontend
            echo json_encode($id != 0 ? ["success" => true] : ["success" => false]);
    }
