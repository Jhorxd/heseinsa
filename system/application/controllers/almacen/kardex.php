...
class Kardex extends controller
{
    ...
    public function reportKardex($startDate, $endDate, $productId, $storeId)
    {
        ...
        if ($startDate == "" || $endDate == "") {
            $periodoTexto = "Sin rango de fechas definido";
        } else {
            // Formatear fechas al estilo "01 de Julio de 2025"
            setlocale(LC_TIME, 'es_ES.UTF-8'); // Puede requerir configuración del sistema

            $fechaInicio = DateTime::createFromFormat('Y-m-d', $startDate);
            $fechaFin = DateTime::createFromFormat('Y-m-d', $endDate);

            // Si prefieres formato sin usar setlocale (más compatible)
            $meses = [
                "01" => 'Enero',
                "02" => 'Febrero',
                "03" => 'Marzo',
            ...
            $inicioTexto = $fechaInicio->format('d') . ' de ' . $meses[$fechaInicio->format('m')] . ' de ' . $fechaInicio->format('Y');
            $finTexto = $fechaFin->format('d') . ' de ' . $meses[$fechaFin->format('m')] . ' de ' . $fechaFin->format('Y');

            $periodoTexto = "Del $inicioTexto al $finTexto";
        }

        $this->excel->setActiveSheetIndex($hoja)->setCellValue("A$lugar", "Periodo:")->mergeCells("A$lugar:D$lugar");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("E$lugar", $periodoTexto);
        $lugar++;
        ...
        $dataKardexUltimoStock = $this->kardex_model->consultar_kardex($filter);
