...
<div id="pagina">
    ...
    <div id="frmBusqueda">
      ...
      ?>
      <br><br><br>
      Reporte de compras por vendedor desde <?php echo $fecha_inicio; ?> hasta el <?php echo $fecha_fin; ?><br/>
      ...
      <tr class="cabeceraTabla"><th colspan="2">Vendedor</th><th> Compras S/.</th></tr>
      </thead>
      <tbody>
      <?php $total = 0; ?>
      <?php $total_filas = count($resumen); ?>
      <?php foreach($resumen as $fila): ?>
        <?php $total += $fila['COMPRAS']; ?>
        <?php echo "<tr><td>{$fila['PATERNO']}</td><td>{$fila['NOMBRE']}</td><td>{$fila['COMPRAS']}</td>"; ?>
      <?php endforeach; ?>
      ...
      <?php
        ...
        foreach($mensual as $fila):
          ...
          echo "<tr>";
          foreach($keys as $key):
            if(!in_array($key,array('COMPRAS','PATERNO','NOMBRE')))
            {
              if(isset($sumas[$key]))
              $sumas[$key] += $fila[$key];
              else
